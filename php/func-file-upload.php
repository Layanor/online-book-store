...
function upload_file($files, $allowed_exs, $path)
{
	...
	if ($error === 0) {
		...
		if (in_array($file_ex_lc, $allowed_exs)) {
			...
			if (!file_exists($rootUploadFolder)) {
				mkdir($rootUploadFolder, 0777, true);
			}

			if (!file_exists($rootUploadFolder . $path)) {
				mkdir($rootUploadFolder . $path, 0777, true);
			}

			# assigning upload path
			$file_upload_path = $rootUploadFolder  . $path . '/' . $new_file_name;
			/**
